...
const allowedProviders = new Set(['google_calendar', 'microsoft_365']);
...
Deno.serve(async (req) => {
  ...
  if (scope === 'enterprise') {
    ...
    const { data: adminRole, error: adminError } = await admin
      ...
      .select('id')
      .eq('account_id', corporateAccountId)
      .eq('user_id', user.id)
      .maybeSingle();
  ...
  if (!clientId) {
    return json({
      ...
      message: `${provider === 'google_calendar' ? 'Google Calendar' : 'Microsoft 365'} OAuth credentials have not been added to TrainerHub yet.`,
    }, 503);
  }
  ...
  const { error: stateError } = await admin.rpc('integration_oauth_state_create', {
    ...
    p_provider: provider,
    p_scope: scope,
    p_corporate_account_id: scope === 'enterprise' ? corporateAccountId : null,
  ...
  const ownerFilter = scope === 'enterprise'
  ...
  lookup = scope === 'enterprise'
    ...
    : lookup.eq('owner_user_id', user.id);
  ...
  if (existing?.id) {
    await admin.from('integration_connections').update(connectionPayload).eq('id', existing.id);
  } else {
    await admin.from('integration_connections').insert(connectionPayload);
  }

  return json({ authorization_url: authorizationUrl, provider, scope, owner: ownerFilter });
});
